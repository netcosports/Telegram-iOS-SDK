...
@interface MTHttpWorker ()
{
    ...
    TGAFHTTPRequestOperation *_operation;
...
- (instancetype)initWithDelegate:(id<MTHttpWorkerDelegate>)delegate address:(MTDatacenterAddress *)address payloadData:(NSData *)payloadData performsLongPolling:(bool)performsLongPolling
{
    ...
    if (self != nil)
    {
        ...
        [[MTHttpWorker httpWorkerProcessingQueue] dispatchOnQueue:^
        {
            ...
            _operation = [[TGAFHTTPRequestOperation alloc] initWithRequest:urlRequest];
            ...
            [_operation setCompletionBlockWithSuccess:^(TGAFHTTPRequestOperation *operation, __unused id responseObject)
            {
                MTHttpWorker *strongSelf = weakSelf;
...
- (void)stop
{
    ...
    TGAFHTTPRequestOperation *operation = _operation;
    _operation = nil;
