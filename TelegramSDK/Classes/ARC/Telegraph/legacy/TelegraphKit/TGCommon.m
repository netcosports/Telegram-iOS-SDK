...
{
    return [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, true)[0] stringByAppendingPathComponent:@"CustomLocalization.bundle"];
}
...
NSString *TGLocalized(NSString *s)
{
    ...
    {
        fallbackBundle = [NSBundle bundleWithPath:[mainBundle() pathForResource:@"en" ofType:@"lproj"]];

        NSString *language = [[NSLocale preferredLanguages] objectAtIndex:0];

        if (![[mainBundle() localizations] containsObject:language])
        {
            ...
            if ([language rangeOfString:@"-"].location != NSNotFound)
            {
                ...
                for (NSString *localization in [mainBundle() localizations])
                {
                    ...
                    {
                        NSBundle *candidateBundle = [NSBundle bundleWithPath:[mainBundle() pathForResource:localization ofType:@"lproj"]];
...
NSBundle *mainBundle() {
    return [NSBundle bundleWithIdentifier:@"com.netcosports.TelegramSDK"];
};
