...
- (instancetype)initWithMessage:(TGMessage *)message imageInfo:(TGImageInfo *)imageInfo author:(TGUser *)author context:(TGModernViewContext *)context
{
    ...
    if (self != nil)
    {
        ...
        dispatch_once(&onceToken, ^
        {
            ...
            placeholderImage = [[UIImage imageNamed:@"ModernMessageImagePlaceholder.png" inBundle:mainBundle() compatibleWithTraitCollection:nil] stretchableImageWithLeftCapWidth:16 topCapHeight:16];
        });
...
- (UIImage *)dateBackground
{
    ...
    dispatch_once(&onceToken, ^
    {
        dateBackgroundImage = [[UIImage imageNamed:@"ModernMessageImageDateBackground.png" inBundle:mainBundle() compatibleWithTraitCollection:nil] stretchableImageWithLeftCapWidth:9 topCapHeight:9];
    });
...
- (UIImage *)checkPartialImage
{
    ...
    dispatch_once(&onceToken, ^
    {
        checkPartialImage = [UIImage imageNamed:@"ModernMessageCheckmarkMedia2.png" inBundle:mainBundle() compatibleWithTraitCollection:nil];
    });
...
- (UIImage *)checkCompleteImage
{
    ...
    dispatch_once(&onceToken, ^
    {
        checkCompleteImage = [UIImage imageNamed:@"ModernMessageCheckmarkMedia1.png" inBundle:mainBundle() compatibleWithTraitCollection:nil];
    });
...
- (void)setTemporaryHighlighted:(bool)temporaryHighlighted viewStorage:(TGModernViewStorage *)__unused viewStorage
{
    if ([_imageModel boundView] != nil)
    {
        if (temporaryHighlighted)
        {
            ...
            {
                UIImage *highlightImage = [UIImage imageNamed:@"ModernImageBubbleHighlight.png" inBundle:mainBundle() compatibleWithTraitCollection:nil];
...
- (TGModernImageViewModel *)unsentButtonModel
{
    if (_unsentButtonModel == nil)
    {
        ...
        dispatch_once(&onceToken, ^
        {
            image = [UIImage imageNamed:@"ModernMessageUnsentButton.png" inBundle:mainBundle() compatibleWithTraitCollection:nil];
        });
