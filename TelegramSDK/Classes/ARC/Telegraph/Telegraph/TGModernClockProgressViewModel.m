...
+ (CGImageRef)frameImageForType:(TGModernClockProgressType)type
{
    switch (type)
    {
        case TGModernClockProgressTypeOutgoingClock:
        {
            ...
            {
                UIImage *rawImage =[UIImage imageNamed:@"ClockFrame.png" inBundle:mainBundle() compatibleWithTraitCollection:nil];
        ...
        case TGModernClockProgressTypeOutgoingMediaClock:
        {
            ...
            dispatch_once(&onceToken, ^
            {
                UIImage *rawImage =[UIImage imageNamed:@"ClockWhiteFrame.png" inBundle:mainBundle() compatibleWithTraitCollection:nil];
                image = CGImageRetain(TGScaleAndRoundCorners(rawImage, CGSizeMake(rawImage.size.width, rawImage.size.height), CGSizeZero, 0, nil, false, nil).CGImage);
            });
...
+ (CGImageRef)minImageForType:(TGModernClockProgressType)type
{
    switch (type)
    {
        case TGModernClockProgressTypeOutgoingClock:
        {
            ...
            dispatch_once(&onceToken, ^
            {
                UIImage *rawImage =[UIImage imageNamed:@"ClockMin.png" inBundle:mainBundle() compatibleWithTraitCollection:nil];
                image = CGImageRetain(TGScaleAndRoundCorners(rawImage, CGSizeMake(rawImage.size.width, rawImage.size.height), CGSizeZero, 0, nil, false, nil).CGImage);
            });
        ...
        case TGModernClockProgressTypeOutgoingMediaClock:
        {
            ...
            dispatch_once(&onceToken, ^
            {
                UIImage *rawImage =[UIImage imageNamed:@"ClockWhiteMin.png" inBundle:mainBundle() compatibleWithTraitCollection:nil];
                image = CGImageRetain(TGScaleAndRoundCorners(rawImage, CGSizeMake(rawImage.size.width, rawImage.size.height), CGSizeZero, 0, nil, false, nil).CGImage);
            });
...
+ (CGImageRef)hourImageForType:(TGModernClockProgressType)type
{
    switch (type)
    {
        case TGModernClockProgressTypeOutgoingClock:
        {
            ...
            dispatch_once(&onceToken, ^
            {
                UIImage *rawImage =[UIImage imageNamed:@"ClockHour.png" inBundle:mainBundle() compatibleWithTraitCollection:nil];
                image = CGImageRetain(TGScaleAndRoundCorners(rawImage, CGSizeMake(rawImage.size.width, rawImage.size.height), CGSizeZero, 0, nil, false, nil).CGImage);
            });
        ...
        case TGModernClockProgressTypeOutgoingMediaClock:
        {
            ...
            dispatch_once(&onceToken, ^
            {
                UIImage *rawImage =[UIImage imageNamed:@"ClockWhiteHour.png" inBundle:mainBundle() compatibleWithTraitCollection:nil];
                image = CGImageRetain(TGScaleAndRoundCorners(rawImage, CGSizeMake(rawImage.size.width, rawImage.size.height), CGSizeZero, 0, nil, false, nil).CGImage);
            });
