...
#define TG_TIMESTAMP_MEASURE(x)
    ...
    if (context != NULL || useViews)
    {
        ...
        if (useViews)
        {
            ...
            dispatch_once(&onceToken, ^
            {
                emptyImage = CGImageRetain(TGScaleImageToPixelSize([UIImage imageNamed:@"Transparent.png" inBundle:mainBundle() compatibleWithTraitCollection:nil], CGSizeMake(2, 2)).CGImage);
            });
