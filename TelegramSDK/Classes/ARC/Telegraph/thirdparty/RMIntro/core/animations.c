...
static float animationsTime_local = 0;
...
float t(float start_value, float end_value, float start_animationsTime, float duration, timing_type type)
{
    if (animationsTime>start_animationsTime+duration) {
        return end_value;
    }
    
    if (type==Linear) {
        return start_value + (end_value - start_value)*MINf(duration+start_animationsTime, MAXf(.0, (animationsTime - start_animationsTime))) /duration;
    }
    return start_value + (end_value - start_value)*timing(MINf(duration+start_animationsTime, MAXf(.0, (animationsTime - start_animationsTime))) /duration, type);
}

float t_reversed(float end_value, float start_value, float start_animationsTime, float duration, timing_type type)
{
    if (animationsTime>start_animationsTime+duration) {
        return end_value;
    }
...
float t_local(float start_value, float end_value, float start_animationsTime, float duration, timing_type type)
{
    if (type==Sin) {
        return start_value + (end_value - start_value)*sin(MINf(MAXf((animationsTime_local - start_animationsTime)/duration * M_PI, 0), M_PI));
    }
    
    if (animationsTime_local>start_animationsTime+duration) {
        return end_value;
    }
    
    if (type==Linear) {
        return start_value + (end_value - start_value)*MINf(duration+start_animationsTime, MAXf(.0, (animationsTime_local - start_animationsTime))) /duration;
    }
    return start_value + (end_value - start_value)*timing(MINf(duration+start_animationsTime, MAXf(.0, (animationsTime_local - start_animationsTime))) /duration, type);
}
...
float speedometer_sin()
{
    ...
    return sin(sin(animationsTime*1000*.15*0.08+speedometer)*M_PI)*5;
}
...
double ms0_anim, animationsTime_anim;
int fps_anim;
int count_anim_fps;
...
double anim_pencil_stage_duration, anim_pencil_start_animationsTime, anim_pencil_start_all_animationsTime, anim_pencil_start_all_end_animationsTime;
int anim_pencil_stage;



int anim_bubble_dots_stage, anim_bubble_dots_q, anim_bubble_dots_count;
double anim_bubble_dots_next_animationsTime, anim_bubble_dots_duration;

int anim_bubble_dots_start_period, anim_bubble_dots_end_period;



double anim_videocam_start_animationsTime, anim_videocam_next_animationsTime, anim_videocam_duration, anim_videocam_angle, anim_videocam_old_angle;
double anim_cam_start_animationsTime, anim_cam_next_animationsTime, anim_cam_duration, anim_cam_angle, anim_cam_old_angle;
CPoint anim_cam_position, anim_cam_old_position;
int qShot;
int firstShot;
double anim_camshot_start_animationsTime, anim_camshot_next_animationsTime, anim_camshot_duration, anim_cambutton_start_animationsTime, anim_cambutton_end_animationsTime;


double anim_smile_start_animationsTime1, anim_smile_start_animationsTime2, anim_smile_blink_start_animationsTime;
...
double anim_pin_start_animationsTime, anim_pin_duration;
...
static void draw_ic(int type)
{
    ...
    float rotation;
    float beginanimationsTimeK;
    float commonDelay;
    
    texture_program_type COLOR, LIGHT_COLOR;
    if (type == 0) {
        beginanimationsTimeK=1.5;
    ...
    if (current_page==1 && direct==0) {
        ...
        // t_y = t(0, -200, /*commonDelay + duration_const*(.5-.2)*beginanimationsTimeK*/0, duration_const, EaseOut);
    }
    ...
    if (type==0) {
        ...
        if (current_page==1 && direct==0) {
            t_y = 0;//t(0, -200, /*commonDelay + duration_const*(.5-.2)*beginanimationsTimeK*/0, duration_const, EaseOut);
        }
        else
        {
            t_y = t(200, 0, commonDelay + duration_const*.2*beginanimationsTimeK, duration_const, EaseOut);
        }
        //t_y = t(200, 0, commonDelay + duration_const*.2*beginanimationsTimeK, duration_const, EaseOut);
    }
    else
    {
        ...
        t_y = t(200, 0, commonDelay + duration_const*.3*beginanimationsTimeK, duration_const, EaseOut);
    }
    float pink = 0;
    if (animationsTime_local > anim_pin_start_animationsTime) {
        pink = t_local(0, 1, anim_pin_start_animationsTime, anim_pin_duration, Sin);
        if (animationsTime_local > anim_pin_start_animationsTime + anim_pin_duration) {
            anim_pin_start_animationsTime = animationsTime_local + duration_const * frand(10, 20)*2;
            anim_pin_duration = duration_const * frand(10, 20)*2;
        }
    }
    float pinasin = sin(animationsTime_local*.1) * pink;
    ic_pin_layer.position = xyzMake(ic_pos.x + cos(animationsTime_local*5)*3*pinasin + cloud_scroll_offset, ic_pos.y + sin(animationsTime_local*5)*1.5*pinasin + t_y , 0);
    ...
    // videocam
    if (type==1) {
        if (animationsTime_local > anim_videocam_next_animationsTime) {
            ...
            anim_videocam_start_animationsTime = animationsTime_local;
            anim_videocam_next_animationsTime = animationsTime_local + duration_const*frand(5, 8);
        }
        ic_videocam_layer.rotation = -30 + t_local(anim_videocam_old_angle, anim_videocam_angle, anim_videocam_start_animationsTime, anim_videocam_duration, EaseOut);
        t_y = t(200, 0, commonDelay + duration_const*.5*beginanimationsTimeK, duration_const, EaseOut);
    ...
    else
    {
        ...
        t_y = t(200, 0, commonDelay + duration_const*.1*beginanimationsTimeK, duration_const, EaseOut);
    }
    if (animationsTime_local > anim_cam_next_animationsTime) {
        anim_cam_duration = duration_const*frand(1., 1.5);
        anim_cam_old_angle = anim_cam_angle;
        anim_cam_old_position = anim_cam_position;
        anim_cam_start_animationsTime = animationsTime_local;
        anim_cam_next_animationsTime = animationsTime_local + 10000;//duration_const*frand(5, 8);
        ...
        if (qShot<=0) {
            ...
            anim_cam_next_animationsTime = animationsTime_local + duration_const*frand(5, 8);//frand(1, 2);
        }
        firstShot=1;
        
        anim_camshot_start_animationsTime = animationsTime_local + duration_const * frand(1, 2)*5;
        
        anim_camshot_duration = duration_const * .4;
        if (qShot>0) {
            anim_cambutton_start_animationsTime = anim_camshot_start_animationsTime;
            anim_cambutton_end_animationsTime = anim_camshot_start_animationsTime + (qShot+1)*anim_camshot_duration*2;
            
            //printf("anim_cambutton_start_animationsTime>%f\n", anim_cambutton_start_animationsTime);
            //printf("anim_cambutton_end_animationsTime>%f\n\n", anim_cambutton_end_animationsTime);
        }
    }
    
    ic_cam_layer.rotation = 15 + t_local(anim_cam_old_angle, anim_cam_angle, anim_cam_start_animationsTime, anim_cam_duration, EaseOut);
    ic_cam_layer.position = xyzMake(
                                    ic_pos.x + 0*t_local(anim_cam_old_position.x, anim_cam_position.x, anim_cam_start_animationsTime, anim_cam_duration, EaseOut) + cloud_scroll_offset,
                                    ic_pos.y + 0*t_local(anim_cam_old_position.y, anim_cam_position.y, anim_cam_start_animationsTime, anim_cam_duration, EaseOut)
                                    + t_y,
                                    0);
    mat4x4_layer(ic_matrix, ic_cam_layer, ic_layer_scale, rotation);
    draw_textured_shape(&ic_cam, ic_matrix, COLOR);
    
    if (animationsTime_local > anim_cambutton_start_animationsTime && animationsTime_local < anim_cambutton_end_animationsTime) {
    ...
    float lens_scale;
    lens_scale=1;
    if (qShot>=0 && animationsTime_local > anim_camshot_start_animationsTime) {
        
        lens_scale = t_local(1, 0, anim_camshot_start_animationsTime, anim_camshot_duration, Sin);
        
        if (animationsTime_local > anim_camshot_start_animationsTime + anim_camshot_duration) {
            
            if (qShot<=0) {
                anim_cam_next_animationsTime = animationsTime_local + duration_const*frand(1, 2);
            }
            
            qShot--;
            anim_camshot_start_animationsTime = animationsTime_local + anim_camshot_duration;
        }
    }
    ...
    if (type==0) {
        ...
        {
            t_y = t(200, 0, commonDelay + duration_const*.0*beginanimationsTimeK, duration_const, EaseOut);
        }
        //t_y = t(200, 0, commonDelay + duration_const*.0*beginanimationsTimeK, duration_const, EaseOut);
    }
    else
    ...
    if (anim_smile_stage==0) {
        //printf("stage=0\n");
        smile_laught = t_local(0, 1, anim_smile_start_animationsTime1, anim_smile_fade_duration, Linear);
        if (animationsTime_local > anim_smile_duration*3 + anim_smile_start_animationsTime1) {
            anim_smile_stage=1;
            anim_smile_start_animationsTime2 = animationsTime_local;// + anim_smile_fade_duration;
        }
    }
    
    if (anim_smile_stage==1) {
        //printf("stage=1\n");
        smile_laught = t_local(1, 0, anim_smile_start_animationsTime2, anim_smile_fade_duration, Linear);
        if (animationsTime_local > anim_smile_duration + anim_smile_start_animationsTime2) {
            anim_smile_stage=0;
            
            int zero_or = MAXf(0, irand(-2, 2));
            if (zero_or!=0 && irand(0, 10)>=7) {
                anim_smile_blink_one = 1;
                anim_smile_blink_start_animationsTime = animationsTime_local + duration_const;
            }
            anim_smile_start_animationsTime1 = animationsTime_local + zero_or*frand(10, 20)*duration_const;
        }
        
    }
    
    float y = sin(animationsTime_local*M_PI*10)*1.5*smile_laught;
    ...
    if (animationsTime_local > anim_smile_blink_start_animationsTime+.1) {
        ...
        anim_smile_blink_start_animationsTime = animationsTime_local + blink_pause;
        
        anim_smile_blink_one = 0;
    }
    
    float eye_scale = t_local(1, 0, anim_smile_blink_start_animationsTime, .1, Sin);
    ...
    if (type==0) {
        ...
        {
            t_y = t(200, 0, commonDelay + duration_const*.4*beginanimationsTimeK, duration_const, EaseOut);
        }
        //t_y = t(200, 0, commonDelay + duration_const*.4*beginanimationsTimeK, duration_const, EaseOut);
    }
    else
    ...
    scale=.7 + 0.2*bubble_dots_sin(animationsTime_local*10);
    ...
    scale=.7 + 0.2*bubble_dots_sin(-M_PI*2/3 + animationsTime_local*10);
    if (anim_bubble_dots_stage==0) scale = MAXf(.7, scale);
    ic_bubble_dot.params.scale = xyzMake(scale, scale, scale);
    ic_bubble_dot.params.position=xyzMake(0, -9/2., 0);
    draw_textured_shape(&ic_bubble_dot, ic_matrix, LIGHT_COLOR);
    
    scale=.7 + 0.2*bubble_dots_sin(-M_PI*2/3*2 + animationsTime_local*10);
    if (anim_bubble_dots_stage==0) scale = MAXf(.7, scale);
    ic_bubble_dot.params.scale = xyzMake(scale, scale, scale);
    ic_bubble_dot.params.position=xyzMake(0+8.5, -9/2., 0);
    draw_textured_shape(&ic_bubble_dot, ic_matrix, LIGHT_COLOR);
    ...
    if (anim_pencil_stage==0) {
        ic_pencil_layer.rotation = t_local(0, -5, anim_pencil_start_all_animationsTime, duration_const*.5, EaseOut);
        
        pencil_x = t_local(0, 14, anim_pencil_start_animationsTime, 1.5*0.85, Linear);
        if (animationsTime_local > anim_pencil_start_animationsTime + 1.5*0.85) {
            //printf("anim_pencil_stage==0\n");
            anim_pencil_start_animationsTime=animationsTime_local;
            anim_pencil_stage=1;
        }
    }
    else if (anim_pencil_stage==1)
    {
        pencil_x = t_local(14, 0, anim_pencil_start_animationsTime, 1.5*0.15, Linear);
        
        if (animationsTime_local > anim_pencil_start_animationsTime + 1.5*0.15) {
            if (irand(0, 5)==0) {
                anim_pencil_start_all_end_animationsTime = animationsTime_local;
                anim_pencil_start_animationsTime=animationsTime_local+duration_const*frand(7, 10)*4;
                anim_pencil_stage=2;
    ...
    {
        ic_pencil_layer.rotation = t_local(-5, 0, anim_pencil_start_all_end_animationsTime, duration_const*.5, EaseOut);
        if (animationsTime_local > anim_pencil_start_animationsTime) {
            //printf("anim_pencil_stage==3\n");
            anim_pencil_start_all_animationsTime = animationsTime_local;
            anim_pencil_start_animationsTime=animationsTime_local;
            //printf("anim_pencil_start_animationsTime>%f", anim_pencil_start_animationsTime);
            anim_pencil_stage=0;
        }
    }
    
    float pencil_v = (anim_pencil_stage != 2 ) ? pencil_v_sin(animationsTime_local*2*M_PI*4) : 0;
...
float anim_safe_button_start_animationsTime, anim_safe_shake_start_animationsTime, anim_safe_shake_duration;
...
void draw_safe(int type, float alpha)
{
    ...
    float dx = t_local(0, 1, anim_safe_shake_start_animationsTime, duration_const*2, Sin);
    float dx_sin = sin(animationsTime_local*50)*3*dx;
    ...
    for (i=0; i<4; i++) {
        ...
        if (i==anim_safe_button_q) {
            alpha = t_local(0, 1, anim_safe_button_start_animationsTime, .1, EaseOut);
        }
        
        
        float begin_animationsTime = (i+1)*duration_const*.3;
        scale = t(0, 1, begin_animationsTime, duration_const*.5, EaseOut);
        if (type==1) scale = 1;
    ...
    for (i=0; i<3; i++) {
        for (j=0; j<4; j++) {
            ...
            float begin_animationsTime = j*duration_const*.2+i*duration_const*.2*2;
            scale = t(0, 1, begin_animationsTime, duration_const*.5, EaseOut);
            if (type==1) scale = 1;
            
            if (animationsTime_local > anim_safe_button_start_animationsTime ) {
                if (animationsTime_local > anim_safe_button_start_animationsTime + duration_const*.5) {
                    anim_safe_button_q++;
                    
                    if (anim_safe_button_q==20) {
                        anim_safe_button_start_animationsTime = animationsTime_local + duration_const*.5*3;
                        anim_safe_shake_start_animationsTime = anim_safe_button_start_animationsTime;
                        anim_safe_button_i = 2;
                        anim_safe_button_j = 3;
                    }
                    else if (anim_safe_button_q==21)
                    {
                        anim_safe_button_start_animationsTime = animationsTime_local+ duration_const*.5*20;
                        anim_safe_button_i = 2;
                        anim_safe_button_j = 3;
                    }
                    else if (anim_safe_button_q>21)
                    ...
                    {
                        anim_safe_button_start_animationsTime = animationsTime_local + duration_const*.5;
                        do
                        {
                            anim_safe_button_i = irand(0, 2);
                            anim_safe_button_j = irand(0, 3);
            ...
            if (animationsTime > begin_animationsTime + duration_const*.5 && i==anim_safe_button_i && j==anim_safe_button_j) {
                scale = t_local(1, 0, anim_safe_button_start_animationsTime, duration_const, Sin);
            }
...
void on_draw_frame() {
    ...
    if (prev_page!=current_page) {
        ...
        if (current_page==4) {
            ...
            anim_safe_button_i = 2;
            anim_safe_button_j = 3;
            anim_safe_button_start_animationsTime=animationsTime_local + 1.5;
        }
    ...
    if (current_page==0) {
        ...
        scale = 1;//t(.8, 1, 0, duration_const*.5, EaseOut) - .23*2 * pow(e, -0.055*animationsTime*k) * cos(0.08*animationsTime*k);
        ...
        if (direct==0) {
            ...
            float e= 2.71;
            float k=100*2;
            scale = 1;//t(.8, 1, 0, duration_const*.5, EaseOut)*0+1 - .23*2 * pow(e, -0.055*animationsTime*k) * cos(0.08*animationsTime*k);
        ...
        float tt = MINf(0, -M_PI*125./180. + animationsTime * M_PI * 2 * 1.5);
    ...
    if (current_page==4) {
        ...
        if (direct==1) {
            ...
            if (animationsTime > duration_const*.8) {
                rglNormalDraw();
        ...
        {
            if (animationsTime > duration_const*.8*private_back_k) {
                rglNormalDraw();
    ...
    if (current_page==0) {
        ...
        if (direct==0) {
            ...
            float tt = MINf(0, -M_PI*125./180. + animationsTime * M_PI * 2 * 1.5);
            //float tt = animationsTime * M_PI*2*1.5;
    ...
    if (current_page==2) {
        ...
        else
        {
            ...
            // if (animationsTime - (.9*duration_const*infinityDurK + duration_const*.25)*2<0) {
    ...
    if (current_page==3) {
        ...
        if (direct==1) {
            ...
            float infinityDurK = 1.1;
            if (animationsTime < duration_const*infinityDurK-.025) {
    ...
    if (current_page==4) {
        ...
        else
        {
            ...
            if (animationsTime<duration_const*.4) {
